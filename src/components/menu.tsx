...
import { ipcRenderer, clipboard, Menu as AppMenu } from "electron";
...
import { LogLevel, useSetting } from "../contexts/setting-context"; 
import { useWorkspace } from "../contexts/workspace-context";
import * as b3util from "../misc/b3util";
import { clearDebugLog, getDebugLogFilePath, getDebugLogSize } from "../misc/log";
...
const getFocusedWebContents = () => {
    // 在渲染进程中，我们可以直接使用 webContents 的方法
    // 这些方法在渲染进程中是可用的
    return {
...
export const Menu: FC<LayoutProps> = () => {
    ...
    useEffect(() => {
        ...
        ipcRenderer.invoke("get-app-name").then(setAppName);
    }, []);
    const { settings, setLayout, setLogLevel, getLogLevel } = useSetting(
        useShallow((state: any) => ({
        settings: state.data,
        setLayout: state.setLayout,
        setLogLevel: state.setLogLevel,
        getLogLevel: state.getLogLevel,
        }))
    );
    ...
    const menuTemplate: MenuItemConstructorOptions[] = useMemo(() => {
        const recentWorkspaces: MenuItemConstructorOptions[] = settings.recent.map((path: string, i: number) => ({
            ...
            : []),
        {
            ...
            submenu: [
            {
                ...
                if (!result.canceled && result.filePath) {
                    fs.writeFileSync(result.filePath, JSON.stringify(b3util.createNewTree(result.filePath), null, 2));
                    workspace.open(result.filePath);
                }
                },
            },
            {
        ...
        },
        {
            ...
            submenu: [
                ...
                { type: "separator" },
                {
                id: "menu.debug.logLevel",
                label: t("logLevel"),
                submenu: [
                    {
                    id: "menu.debug.logLevel.debug",
                    label: t("logLevel.debug"),
                    type: "radio",
                    checked: getLogLevel() === LogLevel.DEBUG,
                    click: () => {
                        setLogLevel(LogLevel.DEBUG);
                    },
                    },
                    {
                    id: "menu.debug.logLevel.info",
                    label: t("logLevel.info"),
                    type: "radio",
                    checked: getLogLevel() === LogLevel.INFO,
                    click: () => {
                        setLogLevel(LogLevel.INFO);
                    },
                    },
                    {
                    id: "menu.debug.logLevel.warn",
                    label: t("logLevel.warn"),
                    type: "radio",
                    checked: getLogLevel() === LogLevel.WARN,
                    click: () => {
                        setLogLevel(LogLevel.WARN);
                    },
                    },
                    {
                    id: "menu.debug.logLevel.error",
                    label: t("logLevel.error"),
                    type: "radio",
                    checked: getLogLevel() === LogLevel.ERROR,
                    click: () => {
                ...
                {
                id: "menu.debug.clearDebugLog",
                label: t("clearDebugLog"),
                enabled: true,
                click: async () => {
                    const logSize = getDebugLogSize();
                    const sizeText = logSize > 0 ? ` (${(logSize / 1024).toFixed(1)} KB)` : "";
                    
                    const result = await ipcRenderer.invoke("show-message-box", {
                    type: "question",
                    buttons: [t("confirm"), t("cancel")],
                    defaultId: 0,
                    message: t("clearDebugLogConfirm") + sizeText,
                    });
                    if (result.response === 0) {
                        ...
                        await ipcRenderer.invoke("show-message-box", {
                            type: "info",
                            message: t("clearDebugLogSuccess"),
                        });
                        } else {
                        await ipcRenderer.invoke("show-message-box", {
                            type: "error",
                            message: t("clearDebugLogFailed"),
                        });
                        }
                    }
                },
                ...
                {
                id: "menu.debug.showDebugLog",
                label: t("showDebugLog"),
                enabled: true,
                click: () => {
                    const logPath = getDebugLogFilePath();
                const logSize = getDebugLogSize();
                    const sizeText = logSize > 0 ? ` (${(logSize / 1024).toFixed(1)} KB)` : " (0 KB)";
                    
                    ipcRenderer.invoke("show-message-box", {
                    type: "info",
                    buttons: [t("openFolder"), t("copyPath"), t("close")],
                    defaultId: 0,
                    message: t("debugLogLocation"),
                    detail: logPath + sizeText,
                    }).then((result) => {
                    if (result.response === 0) {
                        // 打开文件夹
                        ipcRenderer.invoke("show-item-in-folder", logPath);
            ...
            { type: "separator" },
            {
                ...
                label: t("actualSize"),
                click: async () => {
                await ipcRenderer.invoke("set-zoom-factor", 1);
                },
            },
            {
                id: "menu.view.zoomIn",
                label: t("zoomIn"),
                click: async () => {
                const currentZoom = await ipcRenderer.invoke("get-zoom-factor");
                await ipcRenderer.invoke("set-zoom-factor", currentZoom + 0.1);
                },
            },
            {
                ...
                click: async () => {
                const currentZoom = await ipcRenderer.invoke("get-zoom-factor");
                await ipcRenderer.invoke("set-zoom-factor", Math.max(0.1, currentZoom - 0.1));
                },
            },
            { type: "separator" },
            {
