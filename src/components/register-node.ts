...
import { Constructor, ExpressionEvaluator, getNodeType, isExprType, NodeData, NodeDef, NodeLayout } from "../misc/b3type";
import * as b3util from "../misc/b3util";
import i18n from "../misc/i18n";
import { isMacos } from "../misc/keys";
import { logDebug } from "../misc/log";
...
const foundUndefined = (value?: string[]) => {
    ...
    const undefinedVars = value.filter((v) => v && b3util.usingVars?.[v] === undefined);
    if (undefinedVars.length > 0) {
        logDebug('NodeRender', '发现未定义的变量:', {
            undefinedVars,
            allVars: value,
            usingVars: Object.keys(b3util.usingVars || {})
        });
        return true;
    }
    return false;
...
const foundUndefinedInArgs = (def: NodeDef, data: NodeData) => {
    ...
    for (let i = 0; i < def.args.length; i++) {
        ...
        let expr: string | string[] | undefined;
        if (argData && typeof argData === 'object' && 'name' in argData && 'type' in argData) {
            // 新格式：{ name: string, type: ArgType }，表达式在 name 字段中
            expr = (argData as any).name as string | string[] | undefined;
        } else {
            // 传统格式：{ value: unknown }
            expr = (argData as any)?.value as string | string[] | undefined;
        }
        ...
        const parsedExpr = b3util.parseExpr(expr);
        if (foundUndefined(parsedExpr)) {
            logDebug('NodeRender', `节点参数中发现未定义变量 - 节点: ${data.name}, 参数: ${arg.name}`, {
                expr,
                parsedExpr,
                argName: arg.name,
                argType: arg.value_type
            });
            return true;
        }
        } else if (Array.isArray(expr)) {
        for (const str of expr) {
            const parsedExpr = b3util.parseExpr(str);
            if (foundUndefined(parsedExpr)) {
                logDebug('NodeRender', `节点参数数组中发现未定义变量 - 节点: ${data.name}, 参数: ${arg.name}`, {
                    expr: str,
                    parsedExpr,
                    argName: arg.name,
                    argType: arg.value_type
      ...
      expr = (argData as any)?.value as string | string[] | undefined;
        ...
        if (typeof expr === "string") {
            if (!new ExpressionEvaluator(expr).dryRun()) {
                logDebug('NodeRender', `节点表达式验证失败 - 节点: ${data.name}, 参数: ${arg.name}`, {
                    expr,
                    argName: arg.name,
                    argType: arg.value_type
                });
            ...
            if (!new ExpressionEvaluator(str).dryRun()) {
                logDebug('NodeRender', `节点表达式数组验证失败 - 节点: ${data.name}, 参数: ${arg.name}`, {
                    expr: str,
                    argName: arg.name,
                    argType: arg.value_type
                });
            ...
            logDebug('NodeRender', `节点表达式解析异常 - 节点: ${data.name}, 参数: ${arg.name}`, {
                expr,
                argName: arg.name,
                argType: arg.value_type,
                error: e
...
class TreeNode extends Rect {
    ...
    private drawArgsText(attributes: Required<RectStyleProps>, container: Group) {
        ...
        const { str, line } =
        args && (Array.isArray(args) ? args.length > 0 : Object.keys(args).length > 0)
            ? toBreakWord(`${i18n.t("regnode.args")}${Array.isArray(args) ? args.map(arg => arg.name).join(", ") : Object.keys(args).join(", ")}`, 200)
            : { str: "", line: 0 };
        this.upsert(
        "args-bg",
        GRect,
    ...
    private drawInputText(attributes: Required<RectStyleProps>, container: Group) {
        ...
        const { str, line } =
        input.length > 0
            ? toBreakWord(`${i18n.t("regnode.input")}${input.map(v => v.name).join(", ")}`, 200)
            : { str: "", line: 0 };
        this.upsert(
        "input-bg",
        GRect,
    ...
    private drawOutputText(attributes: Required<RectStyleProps>, container: Group) {
        ...
        const { str, line } =
        output.length > 0
            ? toBreakWord(`${i18n.t("regnode.output")}${output.map(v => v.name).join(", ")}`, 200)
            : { str: "", line: 0 };
        this.upsert(
        "output-bg",
        GRect,
    ...
    render(attributes?: Required<RectStyleProps> | undefined, container?: Group): void {
        ...
        // 详细的错误检测和调试日志
        logDebug('NodeRender', `开始检测节点 ${data.name} (ID: ${data.id})`, {
            nodeName: data.name,
            nodeId: data.id,
            nodePath: data.path,
            nodeType: classify
        ...
        const nodeDefExists = b3util.nodeDefs.has(data.name);
        logDebug('NodeRender', `节点定义检查 ${data.name}`, { nodeDefExists, availableNodes: Array.from(b3util.nodeDefs.keys()) });

        const hasPathButNoChildren = data.path && !node.children?.length;
        logDebug('NodeRender', `路径子节点检查 ${data.name}`, { hasPathButNoChildren, path: data.path, childrenCount: node.children?.length });

        const groupNotEnabled = nodeDef?.group && !nodeDef.group.some((g) => b3util.usingGroups?.[g]);
        logDebug('NodeRender', `组启用检查 ${data.name}`, { groupNotEnabled, nodeGroups: nodeDef?.group, usingGroups: b3util.usingGroups });

        const treeDataInvalid = !b3util.checkTreeData(data);
        logDebug('NodeRender', `树数据有效性检查 ${data.name}`, { treeDataInvalid, treeData: data });

        const inputUndefined = foundUndefined(data.input?.map(v => v.name));
        logDebug('NodeRender', `输入变量检查 ${data.name}`, { inputUndefined, inputVars: data.input?.map(v => v.name) });

        const outputUndefined = foundUndefined(data.output?.map(v => v.name));
        logDebug('NodeRender', `输出变量检查 ${data.name}`, { outputUndefined, outputVars: data.output?.map(v => v.name) });

        const argsUndefined = foundUndefinedInArgs(nodeDef, data);
        logDebug('NodeRender', `参数未定义检查 ${data.name}`, { argsUndefined, args: data.args, nodeDefArgs: nodeDef?.args });

        const exprError = hasErrorInArgExpr(nodeDef, data);
        logDebug('NodeRender', `表达式错误检查 ${data.name}`, { exprError, args: data.args });

        if (
        !nodeDefExists ||
        hasPathButNoChildren ||
        groupNotEnabled ||
        treeDataInvalid ||
        inputUndefined ||
        outputUndefined ||
        ...
        // 添加调试日志
        logDebug('NodeRender', `节点 ${data.name} (ID: ${data.id}) 显示为红色，错误原因:`, {
            nodeDefExists,
            hasPathButNoChildren,
            groupNotEnabled,
            treeDataInvalid,
            ...
            nodeId: data.id,
            nodePath: data.path,
            nodeGroup: nodeDef?.group,
            usingGroups: b3util.usingGroups,
            inputVars: data.input?.map(v => v.name),
            outputVars: data.output?.map(v => v.name),
            args: data.args
        });
        ...
        // 节点正常，记录成功状态
        logDebug('NodeRender', `节点 ${data.name} (ID: ${data.id}) 渲染正常`, {
            classify,
            color,
            nodeName: data.name,
            nodeId: data.id,
